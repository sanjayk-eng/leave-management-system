...
import { LeaveTimingSection } from "./shared/LeaveTimingSection";
import { LeaveReasonSection } from "./shared/LeaveReasonSection";
...
const getStatusBadgeClass = (status: string): string => {
  switch (status.toUpperCase()) {
    ...
    case 'WITHDRAWN':           return 'bg-amber-600 text-white';
    case 'WITHDRAWAL_PENDING':  return 'bg-purple-500 text-white';
...
export const MonthlyCalendar = ({ currentDate, leaves, holidays, birthdays = [] }: MonthlyCalendarProps) => {
  const [selectedLeave, setSelectedLeave] = useState<Leave | null>(null);
  const [selectedHoliday, setSelectedHoliday] = useState<Holiday | null>(null);
  const [selectedDateLeaves, setSelectedDateLeaves] = useState<{ leaves: Leave[], date: Date } | null>(null);
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-7 grid-rows-6 gap-1 sm:gap-2">
        {days.map((day, index) => {
          ...
          const dayBirthdays = getBirthdaysForDate(day.date, birthdays) as UpcomingBirthdayEmployee[];
              ...
              {day.dayNumber && (
                <>
                  ...
                  <div className="space-y-1 sm:space-y-1.5">
                    ...
                    {dayLeaves.slice(0, isMobile ? 2 : 3).map((leave, leaveIdx) => {
                          ...
                          >
                            ...
                            {leave.leave_timing && (
                              <div className="flex items-center gap-1 text-[9px] sm:text-[10px] opacity-90">
                                <span className="bg-white/20 px-1 py-0.5 rounded text-[8px] sm:text-[9px] truncate">⏰ {leave.leave_timing}</span>
                              </div>
                            )}
                          </div>
                        </LeaveHoverCard>
                    ...
                    {dayLeaves.length > (isMobile ? 2 : 3) && (
                      <div 
                        ...
                        onClick={() => day.date && setSelectedDateLeaves({ leaves: dayLeaves, date: day.date })}
                      >
          ...
          <DialogContent className="max-w-[95vw] sm:max-w-md">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                ...
                <Badge className={cn("border-0", getStatusBadgeClass(selectedLeave.status))}>
              ...
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                ...
                <span className="text-sm font-semibold">{selectedLeave.leave_type}</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              ...
              <LeaveTimingSection 
                timingType={selectedLeave.leave_timing_type}
                timing={selectedLeave.leave_timing}
              />
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
              ...
              {selectedLeave.reason && (
                <LeaveReasonSection reason={selectedLeave.reason} />
              )}
            </div>
          </DialogContent>
        </Dialog>
          ...
          <DialogContent className="max-w-[95vw] sm:max-w-md">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-rose-50 dark:bg-rose-950/30 rounded-lg border border-rose-200 dark:border-rose-800">
                ...
                <span className="text-sm font-bold text-rose-600 dark:text-rose-400">{selectedHoliday.name}</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
      ...
      {/* All Leaves for Date Dialog */}
      {selectedDateLeaves && (
        <Dialog open={!!selectedDateLeaves} onOpenChange={() => setSelectedDateLeaves(null)}>
          <DialogContent className="max-w-[95vw] sm:max-w-2xl max-h-[80vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <Calendar className="h-5 w-5" />
                All Leaves on {selectedDateLeaves.date.toLocaleDateString('en-US', { 
                  month: 'long', 
                  day: 'numeric',
                  year: 'numeric'
                })}
                ...
                return (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      ...
                      {leave.leave_timing_type && leave.leave_timing && (
                        <LeaveTimingSection 
                      ...
                      {leave.approval_name && (
                        <div className="flex items-center justify-between p-2 bg-muted/50 rounded-lg">
                          <span className="text-xs text-muted-foreground font-medium">Approved by:</span>
                          <span className="text-sm font-semibold">{leave.approval_name}</span>
                        </div>
                      )}
                    </div>
                  </div>
