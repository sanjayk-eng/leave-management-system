...
interface VisProps {
  ...
  selectedMonth: number; // = associate_month (the actual anchor, not just display)
  currentMonth:  number;
    ...
    <div className="rounded-xl border bg-card shadow-sm overflow-hidden">
      ...
      <div className="flex items-center justify-between px-3 py-2 border-b bg-muted/30">
        ...
        <div className="flex items-center gap-2">
          ...
          {/* Month selector — this IS the associate_month, not just a display filter */}
      ...
      <div className="mx-3 mt-3 rounded-lg border border-primary/20 bg-primary/5 px-3 py-2 flex items-start gap-2">
        ...
        <div className="min-w-0">
          ...
          <p className="text-[10px] text-muted-foreground mt-0.5 leading-snug">
            ...
            Changing the month above updates both this preview and the saved allocation.
          </p>
        </div>
      </div>
            ...
            {MONTH_SHORT.map((m, i) => {
              ...
              return (
                <button
                  key={m}
                  type="button"
                  title={`${MONTH_NAMES[i]}${isToday ? ' (current month)' : ''}${isSelected ? ' — allocation starts here' : ''}`}
                ...
                >
                  ...
                  {/* Dot indicator under selected month */}
                  {isSelected && (
          ...
          {rows.map(row => {
            ...
            return (
              <div key={row.label} className="space-y-1">
                ...
                {/* Progress bar: elapsed (grey) + allocated (colour) */}
                ...
                <p className="text-[10px] text-muted-foreground/60 text-right tabular-nums">
                  {row.annual} × {remaining}/12 = <span className={`font-semibold ${row.text}`}>{fmtDays(row.prorated)}</span> days
                </p>
              </div>
            );
          })}
